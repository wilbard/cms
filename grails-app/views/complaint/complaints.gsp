                ...
                <table class="table tables-min-height">
                    ...
                    <g:if test="${complaints}">
                        <g:each in="${complaints}" var="complaint">
                            <g:set var="escalator_info" value="${manageInfo.escalator_details(complaint.id, user_info.id)}"/>
                            <tr class="${escalator_info != null ? (escalator_info.is_accessed ? 'alert alert-default no-border' : 'alert alert-info no-border' ) : ''}">
